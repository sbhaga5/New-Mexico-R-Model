...
year_base_var = read.csv(file = "year_base_var.csv", header = TRUE, fileEncoding = "UTF-8-BOM")
age_base_var = read.csv(file = "age_base_var.csv", header = TRUE, fileEncoding = "UTF-8-BOM")
#
##################################################################################################################################################################
#
#Load Inputs
...
AmortizationYears[1][1] <- NoYearsADC
OutstandingBase[1,1] <- AccrLiabNewDR[3][1] - AVA[3][1] 
rate <- ((1 + NewDR[3][1]) / (1 + AmoBaseInc)) - 1
period <- AmortizationYears[1,1]
pmt <- 1
Amortization[1,1] <- OutstandingBase[1,1] / (PresentValue(rate,period,pmt) / ((1+NewDR[3][1])^0.5))
#
##################################################################################################################################################################
#
#Projections
...
#for(k in 2:ncol(Scenario_Data)){
for(i in StartIndex:length(FYE)){
  #Payroll
  TotalPayroll[i][1] <- TotalPayroll[i-1][1]*(1 + Payroll_growth)
  PayrollTier1Growth <- (1 - Payroll_growthT1_1) - (Payroll_growthT1_1*Payroll_growthT1_2)*(FYE[i][1] - Payroll_anchor_year)
  PayrollTier2Growth <- (1 - Payroll_growthT2_1) - (Payroll_growthT2_1*Payroll_growthT2_2)*(FYE[i][1]- Payroll_anchor_year)
  PayrollTier3Growth <- (1 - Payroll_growthT3_1) - (Payroll_growthT3_1*Payroll_growthT3_2)*(FYE[i][1] - Payroll_anchor_year)
  Tier1Payroll[i][1] <- Tier1Payroll[i-1][1]*PayrollTier1Growth
  Tier2Payroll[i][1] <- Tier2Payroll[i-1][1]*PayrollTier2Growth
  Tier3Payroll[i][1] <- Tier3Payroll[i-1][1]*PayrollTier3Growth
  NewHirePayroll[i][1] <- TotalPayroll[i][1] - (Tier1Payroll[i][1] + Tier2Payroll[i][1] + Tier3Payroll[i][1])
  ARPPayroll[i][1] <- ARPPayroll[i-1][1]*(TotalPayroll[i][1] / TotalPayroll[i-1][1])
  #
  #Discount Rate
  OriginalDR[i][1] <- dis_r
  NewDR[i][1] <- dis_r_proj
  #
  #Benefit Payments, Admin Expenses
  count <- i - StartIndex + 1
  BenPayments[i][1] <- -1*sum(BP_Matrix[count,])
  AdminExp[i][1] <- -1*Admin_Exp_Percentage*TotalPayroll[i][1]
  #
  #Accrued Liability, MOY NC - Original DR
  if(i > (StartIndex)){
    NCGrowth <- (1 + NCAnchor_growth)^(FYE[i][1]  - NC_StaryYear + 1)
    MOYNCExistOrigDR[i-1][1] <- (Tier1Payroll[i][1]*(NC_Tier1) + Tier2Payroll[i][1]*(NC_Tier2) + Tier3Payroll[i][1]*(NC_Tier3_4))*NCGrowth
    MOYNCNewHireOrigDR[i-1][1] <- NewHirePayroll[i][1]*(NCAnchor_NewHire)*NCGrowth
  }
  AccrLiabOrigDR[i][1] <- AccrLiabOrigDR[i-1][1]*(1+OriginalDR[i-1][1]) + (MOYNCExistOrigDR[i-1][1] + MOYNCNewHireOrigDR[i-1][1] + BenPayments[i][1])*(1+OriginalDR[i-1][1])^0.5
  #
  #Accrued Liability, MOY NC - New DR
  DRDifference <- 100*(OriginalDR[i][1]-NewDR[i][1])
  if(i > (StartIndex)){
    MOYNCExistNewDR[i-1][1] <- MOYNCExistOrigDR[i-1][1]*(1+(NCSensDR/100))^(DRDifference)
    MOYNCNewHireNewDR[i-1][1] <- MOYNCNewHireOrigDR[i-1][1]*(1+(NCSensDR/100))^(DRDifference) 
  }
  AccrLiabNewDR[i][1] <- AccrLiabOrigDR[i][1]*((1+(LiabSensDR/100))^(DRDifference))*((1+(Convexity/100))^((DRDifference)^2/2))
  #
  #Contribution Policy, Amortization Policy
  if((FYE[i][1] < 2026) && (ContrFreeze == 'FREEZE')){
    EE_Contrib[i][1]<- EE_Contrib[i-1][1]
    ER_NC[i][1] <- ER_NC[i-1][1]
    ER_ARP[i][1] <- ER_ARP[i-1][1]
    ER_Amo[i][1] <- ER_Amo[i-1][1]
  } else {
    EE_Contrib[i][1] <- TotalPayroll[i][1]*(EE_Over24K*(EEContrib - EEContrib_24K) + EEContrib_24K)
    ER_NC[i][1] <- MOYNCExistNewDR[i-1][1] + MOYNCNewHireNewDR[i-1][1] - AdminExp[i][1] - EE_Contrib[i][1]
    ER_ARP[i][1] <- ARPPayroll[i][1]*ER_ARP_Percentage
    
    if(ER_Policy == 'Statutory Rate'){
      ER_Amo[i][1] <- ERContrib*TotalPayroll[i][1] - ER_NC[i][1]
    } else {
      if(count > nrow(Amortization)){
        ER_Amo[i][1] <- max(0 - ER_ARP[i][1], -(ER_NC[i][1] + ER_ARP[i][1]))
      } else {
        ER_Amo[i][1] <- max(sum(Amortization[count,]) - ER_ARP[i][1], -(ER_NC[i][1] + ER_ARP[i][1]))
      }
    }
  }
  CashFlows <- (BenPayments[i][1] + AdminExp[i][1] + EE_Contrib[i][1] + ER_NC[i][1] + ER_ARP[i][1] + ER_Amo[i][1])
  ROA_MVA[i][1] <- as.double(Scenario_Data[count,ScenarioIndex])
  Solv_Contrib[i][1] <- max(-(MVA[i-1][1]*(1+ROA_MVA[i][1]) + CashFlows*(1+ROA_MVA[i][1])^0.5) / (1+ROA_MVA[i][1])^0.5,0)
  Total_ER[i][1] <- ER_NC[i][1] + ER_Amo[i][1] + ER_ARP[i][1] + Solv_Contrib[i][1]
  Total2021_ER[i][1] <- Total_ER[i][1] / ((1 + asum_infl)^(FYE[i][1] - NC_StaryYear))
  #
  #Net CF, Expected MVA
  NetCF[i][1] <- CashFlows + Solv_Contrib[i][1]
  ExpInvInc[i][1] <- (MVA[i-1][1]*NewDR[i-1][1]) + (NetCF[i][1]*NewDR[i-1][1]*0.5)
  ExpectedMVA[i][1] <- MVA[i-1][1] + NetCF[i][1] + ExpInvInc[i][1]
  MVA[i][1] <- MVA[i-1][1]*(1+ROA_MVA[i][1]) + NetCF[i][1]*(1+ROA_MVA[i][1])^0.5
  #
  #Gain Loss, Defered Losses
  GainLoss[i][1] <- MVA[i][1] - ExpectedMVA[i][1] 
  DeferedCurYear[i][1] <- GainLoss[i][1]*(0.8/1)
  Year1GL[i][1] <- DeferedCurYear[i-1][1]*(0.6/0.8)
  Year2GL[i][1] <- Year1GL[i-1][1]*(0.4/0.6)
  Year3GL[i][1] <- Year2GL[i-1][1]*(0.2/0.4)
  TotalDefered[i][1] <- Year1GL[i][1] + Year2GL[i][1] + Year3GL[i][1]
  #
  AVA_Temp <- as.double(MVA[i][1] - TotalDefered[i][1])
  AVA_Temp_1 <- as.double(MVA[i][1])
  AVA[i][1] <- min(max(AVA_Temp, AVA_Temp_1*AVA_lowerbound), AVA_Temp_1*AVA_upperbound)
  UAL_AVA[i][1] <- AccrLiabNewDR[i][1] - AVA[i][1]
  FR_AVA[i][1] <- AVA[i][1]/AccrLiabNewDR[i][1]
  UAL_MVA[i][1] <- AccrLiabNewDR[i][1] - MVA[i][1]
  FR_MVA[i][1] <- MVA[i][1]/AccrLiabNewDR[i][1]
  #
  #Amo Policy, Outstanding Balance
  #Amo Years
  if(Amo_Type == 'Layered'){
    AmortizationYears[count+1][1] <- NoYearsADC
  } else {
    AmortizationYears[count+1][1] <- max(AmortizationYears[count+1][1] - 1,0)
  }
  
  if(count < nrow(Amortization)){
    #Oustanding Balance
    for(j in 2:(count + 1)){
      OutstandingBase[count+1,j] <- OutstandingBase[count,j-1]*(1 + NewDR[i-1][1]) - (Amortization[count,j-1]*(1 + NewDR[i-1][1])^0.5)
    }
    OutstandingBase[count+1,1] <- AccrLiabNewDR[i][1] - AVA[i][1] - sum(OutstandingBase[count+1,2:ncol(OutstandingBase)])
    
    #Amo Layers
    for(j in 1:(count + 1)){
      rate <- ((1 + NewDR[i][1]) / (1 + AmoBaseInc)) - 1
      period <- max(AmortizationYears[count+1,1] - j + 1, 1)
      pmt <- 1
      Amortization[count+1,j] <- OutstandingBase[count+1,j] / (PresentValue(rate,period,pmt) / ((1+NewDR[i-1][1])^0.5))
    }
  }
}
...
#Scenario_ER <- cbind(Scenario_FR,Total2021_ER)
#}
