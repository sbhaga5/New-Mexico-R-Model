...
RunModel <- function(user_inputs, Historical_Data, Scenario_Data, AnalysisType, SimType, SimReturn, SimVolatility, ScenType, BP_Matrix, AmoYearsInput, Amortization, OutstandingBase, OffsetYears){
  ...
  for(i in StartIndex:length(FYE)){
    ...
    AdminExp[i] <<- -1*Admin_Exp_Percentage*(Tier1Payroll[i] + Tier2Payroll[i] + Tier3Payroll[i])
    #
    ...
    if((FYE[i] < 2026) && (ContrFreeze == 'FREEZE')){
      ...
      EE_Contrib[i] <<- (Tier1Payroll[i] + Tier2Payroll[i] + Tier3Payroll[i])*(EE_Over24K*(EEContrib - EEContrib_24K) + EEContrib_24K)
      ...
      if(ER_Policy == 'Statutory Rate'){
        ER_Amo[i] <<- ERContrib*(Tier1Payroll[i] + Tier2Payroll[i] + Tier3Payroll[i]) - ER_NC[i]
      } else {
    ...
    if(count < nrow(Amortization)){
      ...
      for(j in 1:(count + 1)){
        ...
        period <- max(OffsetYears[count+1,j] - j + 1, 0)
        pmt <- 1
        if(period > 0){
