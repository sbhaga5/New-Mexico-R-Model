...
User_Input_Amo <<- user_inputs[which(user_inputs[,1] == 'User Input for Amortization Layers'),2]
...
SimReturnAssumed <<- as.double(user_inputs[which(user_inputs[,1] == 'Simulation Returns (Arithmetic)'),2])
SimReturnConservative <<- as.double(user_inputs[which(user_inputs[,1] == 'Simulation Returns (Arithmetic)'),3])
SimVolatility <<- as.double(user_inputs[which(user_inputs[,1] == 'Simulation Volatility'),2])
...
for(i in 1:nrow(OffsetYears)){
  RowCount <- i
  ColCount <- 1
  while(RowCount <= nrow(OffsetYears)){
...
#
RunModel <- function(user_inputs, Historical_Data, Scenario_Data, AnalysisType, SimType, SimReturn, SimVolatility, ScenType){
  ##################################################################################################################################################################
  #
...
###############################################################
#RunModel(user_inputs, Historical_Data, Scenario_Data, 'Deterministic','', 'Assumption')
library(ggplot2)
#Scenarios
...
for (i in 1:length(Scenarios)){
  ...
  NewData <- RunModel(user_inputs, Historical_Data, Scenario_Data, 'Deterministic','', SimReturn, SimVolatility, NewScenario)
...
#   NewData <- RunModel(user_inputs, Historical_Data, Scenario_Data, 'Stochastic','Assumed', SimReturn, SimVolatility, '')
